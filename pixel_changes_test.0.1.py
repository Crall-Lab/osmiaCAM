...
If --output is given, runs headlessly and writes directly to an .mp4.
Otherwise it shows the combined plot+image interactively.
...
# Decide whether to use Agg backend (no display) or default interactive
...
def parse_args():
    ...
    p.add_argument('--delay', type=int, default=100,
                   help='Delay between videos in ms (used for fps in output or pause time)')
    p.add_argument('--ymin', type=float, default=None,
                   help='Lower y-limit for diff plot')
    p.add_argument('--ymax', type=float, default=None,
                   help='Upper y-limit for diff plot')
    p.add_argument('--output', '-o', type=str, default=None,
                   help='Path to output .mp4 (optional)')
    return p.parse_args()
...
def main():
    ...
    print("Click two endpoints on the 3rd frame of the first video.")
    ...
    if args.output:
        ...
        if not writer.isOpened():
            print(f"Warning: could not open writer at {args.output}")
            writer = None
    ...
    for vid in videos:
        ...
        # draw and write if needed
        fig.canvas.draw()
        if writer:
            buf = np.asarray(fig.canvas.buffer_rgba())       # (H_buf, W_buf,4)
            rgb = buf[:, :, :3]
            H_buf, W_buf = rgb.shape[:2]
            if (W_buf, H_buf) != (width, height):
                print(f"[Warning] buffer {(W_buf,H_buf)} != writer {(width,height)}, disabling.")
                writer.release(); writer = None
            else:
                frame_bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
                writer.write(frame_bgr)

        # if interactive, pause; otherwise just proceed
        if not args.output:
